...
fn emit_element(a: &elle_plugin::Api, ctx: *mut ElleCtx, val: ElleValue, out: &mut String) {
    ...
    match children_len {
        ...
        Some(len) => {
            out.push('>');
            for i in 0..len {
                emit_element(a, ctx, a.get_array_item(children_val, i), out);
            }
            out.push_str("</");
            out.push_str(&tag);
            out.push('>');
...
fn get_svg_string(ctx: *mut ElleCtx, val: ElleValue, name: &str) -> Result<String, ElleResult> {
    ...
    if let Some(s) = a.get_string(val) {
        ...
        emit_element(a, ctx, val, &mut out);
        Ok(out)
    } else {
        Err(a.err(ctx, 
            "type-error",
