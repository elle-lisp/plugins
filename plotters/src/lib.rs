...
fn parse_color(a: &elle_plugin::Api, ctx: *mut ElleCtx, val: ElleValue) -> Option<RGBColor> {
...
fn parse_opts(a: &elle_plugin::Api, ctx: *mut ElleCtx, val: ElleValue) -> ChartOpts {
    ...
    if let Some(c) = parse_color(a, ctx, a.get_struct_field(val, "color")) {
        o.colors = vec![c];
    }
    let cv = a.get_struct_field(val, "colors");
    if let Some(len) = a.get_array_len(cv) {
        let cs: Vec<_> =
            (0..len).filter_map(|i| parse_color(a, ctx, a.get_array_item(cv, i))).collect();
        if !cs.is_empty() {
            o.colors = cs;
        }
    }
...
extern "C" fn prim_line(ctx: *mut ElleCtx, args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    let opts =
        if nargs > 1 { parse_opts(a, ctx, unsafe { a.arg(args, nargs, 1) }) } else { ChartOpts::default() };
...
extern "C" fn prim_bar(ctx: *mut ElleCtx, args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    let opts =
        if nargs > 2 { parse_opts(a, ctx, unsafe { a.arg(args, nargs, 2) }) } else { ChartOpts::default() };
...
extern "C" fn prim_histogram(ctx: *mut ElleCtx, args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    let opts = if nargs > 1 {
        parse_opts(a, ctx, unsafe { a.arg(args, nargs, 1) })
    } else {
        ChartOpts::default()
    };
...
extern "C" fn prim_chart(ctx: *mut ElleCtx, args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    let opts = parse_opts(a, ctx, spec);
    ...
    for i in 0..slen {
        ...
        let color = parse_color(a, ctx, a.get_struct_field(s, "color"));
