...
fn parse_layer(ctx: *mut ElleCtx, a: &elle_plugin::Api, v: ElleValue) -> zwlr_layer_shell_v1::Layer {
...
fn parse_anchor(ctx: *mut ElleCtx, a: &elle_plugin::Api, v: ElleValue) -> zwlr_layer_surface_v1::Anchor {
...
extern "C" fn prim_layer_surface(ctx: *mut ElleCtx, args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    if nargs >= 2 {
        ...
        if a.check_struct(opts) {
            ...
            if !a.check_nil(lv) {
                layer = parse_layer(ctx, a, lv);
            }

            let av = a.get_struct_field(opts, "anchor");
            if !a.check_nil(av) {
                anchor = parse_anchor(ctx, a, av);
            }
