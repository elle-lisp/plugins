...
pub(crate) extern "C" fn prim_width(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let r = match get_image_ref(a.arg(args, nargs, 0), "image/width") { Ok(r) => r, Err(e) => return e };
    a.ok(a.int(r.with(|img| img.width() as i64)))
}

pub(crate) extern "C" fn prim_height(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let r = match get_image_ref(a.arg(args, nargs, 0), "image/height") { Ok(r) => r, Err(e) => return e };
    a.ok(a.int(r.with(|img| img.height() as i64)))
}

pub(crate) extern "C" fn prim_dimensions(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let r = match get_image_ref(a.arg(args, nargs, 0), "image/dimensions") { Ok(r) => r, Err(e) => return e };
...
pub(crate) extern "C" fn prim_color_type(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let r = match get_image_ref(a.arg(args, nargs, 0), "image/color-type") { Ok(r) => r, Err(e) => return e };
    a.ok(a.keyword(r.with(color_type_keyword)))
}

pub(crate) extern "C" fn prim_pixels(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let img = match get_image(a.arg(args, nargs, 0), "image/pixels") { Ok(i) => i, Err(e) => return e };
    a.ok(a.bytes(img.as_bytes()))
}

pub(crate) extern "C" fn prim_from_pixels(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let w = match require_int(a.arg(args, nargs, 0), "image/from-pixels", "width") { Ok(v) => v as u32, Err(e) => return e };
    let h = match require_int(a.arg(args, nargs, 1), "image/from-pixels", "height") { Ok(v) => v as u32, Err(e) => return e };
...
pub(crate) extern "C" fn prim_get_pixel(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let r = match get_image_ref(a.arg(args, nargs, 0), "image/get-pixel") { Ok(r) => r, Err(e) => return e };
    let x = match require_int(a.arg(args, nargs, 1), "image/get-pixel", "x") { Ok(v) => v as u32, Err(e) => return e };
    let y = match require_int(a.arg(args, nargs, 2), "image/get-pixel", "y") { Ok(v) => v as u32, Err(e) => return e };
    r.with(|img| {
...
pub(crate) extern "C" fn prim_put_pixel(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let x = match require_int(a.arg(args, nargs, 1), "image/put-pixel", "x") { Ok(v) => v as u32, Err(e) => return e };
    let y = match require_int(a.arg(args, nargs, 2), "image/put-pixel", "y") { Ok(v) => v as u32, Err(e) => return e };
    let color = match extract_color(a.arg(args, nargs, 3), "image/put-pixel") { Ok(c) => c, Err(e) => return e };
...
pub(crate) extern "C" fn prim_thaw(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let img = match get_image(a.arg(args, nargs, 0), "image/thaw") { Ok(i) => i, Err(e) => return e };
    a.ok(wrap_image_mut(img))
}

pub(crate) extern "C" fn prim_freeze(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let m = match get_image_mut(a.arg(args, nargs, 0), "image/freeze") { Ok(m) => m, Err(e) => return e };
    a.ok(wrap_image(m.0.borrow().clone()))
}

pub(crate) extern "C" fn prim_new(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let w = match require_int(a.arg(args, nargs, 0), "image/new", "width") { Ok(v) => v as u32, Err(e) => return e };
    let h = match require_int(a.arg(args, nargs, 1), "image/new", "height") { Ok(v) => v as u32, Err(e) => return e };
    let v2 = a.arg(args, nargs, 2);
