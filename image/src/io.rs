...
pub(crate) extern "C" fn prim_read(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let path = match require_string(a.arg(args, nargs, 0), "image/read", "path") { Ok(s) => s, Err(e) => return e };
...
pub(crate) extern "C" fn prim_write(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let img = match get_image(a.arg(args, nargs, 0), "image/write") { Ok(i) => i, Err(e) => return e };
    let path = match require_string(a.arg(args, nargs, 1), "image/write", "path") { Ok(s) => s, Err(e) => return e };
    match img.save(&path) {
        Ok(()) => a.ok(a.nil()),
...
pub(crate) extern "C" fn prim_decode(args: *const ElleValue, nargs: usize) -> ElleResult {
    ...
    let fmt = match parse_format(a.arg(args, nargs, 1), "image/decode") { Ok(f) => f, Err(e) => return e };
...
pub(crate) extern "C" fn prim_encode(args: *const ElleValue, nargs: usize) -> ElleResult {
    let a = api();
    let img = match get_image(a.arg(args, nargs, 0), "image/encode") { Ok(i) => i, Err(e) => return e };
    let fmt = match parse_format(a.arg(args, nargs, 1), "image/encode") { Ok(f) => f, Err(e) => return e };
